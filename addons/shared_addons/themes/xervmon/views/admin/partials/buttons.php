<?php if (isset($buttons) && is_array($buttons)): ?>
	...
	<?php 
		...
		if(isset($button_type) && $button_type == ' btn-primary'):
			$btn_class = 'btn';
		elseif(isset($button_type) && $button_type == ' btn-danger'):
			$btn_class = 'btn';
		else:
			// Default to primary
			$btn_class = 'btn';
	...
	<?php foreach ($buttons as $key => $button): ?>
		...
		<?php switch ($button) :
			...
			case 'save':
			?>
			<button type="submit" name="btnAction" value="<?php echo $button ?>" class="<?php echo $btn_class; ?> btn-success xervmon-btn-big">
					<span><?php echo lang('buttons:' . $button); ?></span>
				</button>
				<?php break;
			...
			case 'close':
			case 'preview':
				if($btn_class == 'btn') $btn_class .= ' cancelButton gray xervmon-btn-big';
