...
jQuery(function ($) {
        ...
        if (typeof CKEDITOR != 'undefined') {
            $.each(CKEDITOR.instances, function (instance) {
                CKEDITOR.instances[instance].on('maximize', function (e) {
                    ...
                    $('.cke_button__maximize').toggleClass('ckeditor-xervmon-logo');
                });
            });
        }
    };
    ...
    pyro.init = function () {
        ...
        $(".check-all").on('click', function () {
            var check_all = $(this),
                all_checkbox = $(this).is('.grid-check-all') ? $(this).parents(".list-items").find(".grid input[type='checkbox']") : $(this).parents("table").find("tbody input[type='checkbox']");
    ...
    pyro.sort_tree = function ($item_list, $url, $cookie, data_callback, post_sort_callback, sortable_opts) {
        ...
        $item_list.find('li').on('click', function () {
            ...
            $item_list.find('li.minus:visible').each(function () {
                items.push('#' + this.id)
            });
    ...
    pyro.chosen = function () {
        ...
        if (!pyro.is_mobile) {
            ...
            $('select:not(.skip)').livequery(function () {
                ...
                try {
                    $this.parent().find(".chosen-container").addClass($this.data("class") || "input-medium");
                } catch (err) {
